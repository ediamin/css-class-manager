...
export interface Selectors {
	...
	getUserSettings: ( state: State ) => UserSettings | undefined;
	getPanelLabel: ( state: State ) => string;
}
...
const store = createReduxStore< State, Actions, Selectors >( STORE_NAME, {
	initialState: {
		...
		userSettings: cssClassManager.userSettings,

		panelLabel: cssClassManager.panelLabel,
	},
	...
	reducer( state: State, action: ReducerAction ) {
		switch ( action.type ) {
			...
			case ACTION_TYPE.SAVE_USER_SETTINGS:
				state = {
					...state,
	...
	actions: {
		...
		async updateUserSettings( updatedSettings: UserSettings ) {
			const actions = dispatch( STORE_NAME ) as Actions;
			...
			try {
				await apiFetch( {
					...
					data: {
						meta: {
							css_class_manager_user_settings: updatedSettings,
						},
					},
				} );

				actions.createSuccessNotice(
					__( 'Preferences updated.', 'css-class-manager' )
				);

				return {
					type: ACTION_TYPE.SAVE_USER_SETTINGS,
